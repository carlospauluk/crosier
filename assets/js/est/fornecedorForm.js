...
$(document).ready(function () {
    ...
    function findPessoaByDocumento(documento) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $('input[id=fornecedor_nome]').val(data.nome);
                $('input[id=fornecedor_pessoa_id]').val(data.id);
                $('input[id=fornecedor_razao_social]').val(data.nome);
                $('input[id=fornecedor_nome_fantasia]').val(data.nomeFantasia);

                $('input[id=fornecedor_fone1]').val(data.fone1);
                $('input[id=fornecedor_email]').val(data.email);
    ...
    $('input[id=fornecedor_cpf]').blur(function () {
        findPessoaByDocumento(this.value);
    });
    $('input[id=fornecedor_cnpj]').blur(function () {
        findPessoaByDocumento(this.value);
    });
